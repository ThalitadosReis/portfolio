...
export default function Contact() {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-x-16 gap-y-10 items-center">
        <div>
          <Reveal>
          ...
          <Reveal
            delay={0.16}
            className="flex gap-4 items-center flex-wrap mt-8"
          >
            <a
              href={`mailto:${siteData.email}`}
        ...
        <Reveal
          delay={0.24}
          className="flex flex-col border-t border-rule mt-2 md:mt-6 md:border-t-0 md:border-l md:border-rule md:pl-10 min-w-[200px]"
        >
          {contactItems.map((item) => (
            <div
              key={item.label}
              className="flex flex-col gap-1.5 border-b border-rule py-5"
            >
